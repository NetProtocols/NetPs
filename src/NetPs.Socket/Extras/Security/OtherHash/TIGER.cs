namespace NetPs.Socket.Extras.Security.OtherHash
{
    ...
    public class TIGER
    {
        ...
        internal static void s_pass(ref ulong a, ref ulong b, ref ulong c, ulong[] x, byte mul)
        {
        ...
        internal static void s_key_schedule(ref ulong[] x)
        {
        ...
        internal static void ProcessBlock(ref TIGER_CTX ctx)
        {
            ulong a, b, c, tmp;
            ulong[] buf = ctx.buf;
            a = ctx.a;
            ...
            s_pass(ref a, ref b, ref c, buf, 5);
            s_key_schedule(ref buf);
            s_pass(ref c, ref a, ref b, buf, 7);
            s_key_schedule(ref buf);
            s_pass(ref b, ref c, ref a, buf, 9);
            for (uint no = 0; no < ctx.passes; no++)
            {
                s_key_schedule(ref buf);
                s_pass(ref a, ref b, ref c, buf, 9);
                tmp = a; a = c; c = b; b = tmp;
            }
        ...
        internal static TIGER_CTX Init()
        {
            var ctx = new TIGER_CTX();
            ctx.a = 0x0123456789ABCDEF;
            ctx.b = 0xFEDCBA9876543210;
            ctx.c = 0xF096A5B4C3B2E187;
            ctx.passes = 0;
            ctx.buffer = ulong_buf_reverse.New(8);
            return ctx;
        }
        internal static void Update(ref TIGER_CTX ctx, byte[] data, int length)
        {
        ...
        internal static byte[] Final(ref TIGER_CTX ctx)
        {
            if (ctx.buffer.IsFULL()) ProcessBlock(ref ctx);
            ctx.buffer.PushNext(0x01);
            if (ctx.buffer.IsFULL(1)) ProcessBlock(ref ctx);
            ctx.buffer.Fill(0, 1);
            ctx.buffer.PushTotal64();
            ProcessBlock(ref ctx);

            var md = new byte[24];
            int i = 0;
            md.CopyFrom(ctx.a, i++ << 3);
            md.CopyFrom(ctx.b, i++ << 3);
            md.CopyFrom(ctx.c, i++ << 3);
            return md;
        }

        public virtual int Passes { get; protected set; }
        public TIGER(int passes)
        {
            this.Passes = passes;
        }
        ...
        {
            var ctx = TIGER.Init();
            ctx.SetPasses(this.Passes);
            TIGER.Update(ref ctx, data, data.Length);
